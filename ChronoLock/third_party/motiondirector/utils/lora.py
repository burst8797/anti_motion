...
def inject_trainable_lora_extended(
    ...
    def pop_matching_lora_pair(lora_list, expected_up_shape, expected_down_shape):
        if lora_list is None or len(lora_list) < 2:
            return None, None
        for i in range(0, len(lora_list) - 1, 2):
            up = lora_list[i]
            down = lora_list[i + 1]
            if tuple(up.shape) == tuple(expected_up_shape) and tuple(down.shape) == tuple(expected_down_shape):
    ...
    if True:
        for target_replace_module_i in target_replace_module:
            for _module, name, _child_module in _find_modules(
                ...
                if loras != None:
                    expected_up = _module._modules[name].lora_up.weight.shape
                    expected_down = _module._modules[name].lora_down.weight.shape
                    up_w, down_w = pop_matching_lora_pair(loras, expected_up, expected_down)
                    if up_w is not None and down_w is not None:
                        _module._modules[name].lora_up.weight = up_w
                        _module._modules[name].lora_down.weight = down_w
